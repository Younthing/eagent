...
- 默认每个 run 生成 `report.html` / `report.docx` / `report.pdf`（可用 `--no-html` / `--no-docx` / `--no-pdf` 关闭）
...
- `results/batch/<relative_path_without_ext>/report.html`
- `results/batch/<relative_path_without_ext>/report.docx`
- `results/batch/<relative_path_without_ext>/report.pdf`

常用参数：
- `--options` / `--options-file` / `--set key=value`
- `--table/--no-table`
- `--html/--no-html` / `--docx/--no-docx` / `--pdf/--no-pdf`（默认全开）
- `--batch-id` / `--batch-name`
