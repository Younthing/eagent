...
def d5_reporting_node(state: dict) -> dict:
    ...
    defaults = get_domain_defaults("D5")
    model_id = str(state.get("d5_model") or defaults["model"] or "").strip()
    model_provider = state.get("d5_model_provider") or defaults["model_provider"]
    temperature = (
        defaults["temperature"]
        if state.get("d5_temperature") is None
        else float(str(state.get("d5_temperature")))
    )
    timeout = (
        defaults["timeout"]
        if state.get("d5_timeout") is None
        else float(str(state.get("d5_timeout")))
    )
    max_tokens = (
        defaults["max_tokens"]
        if state.get("d5_max_tokens") is None
        else int(str(state.get("d5_max_tokens")))
    )
    max_retries = (
        defaults["max_retries"]
        if state.get("d5_max_retries") is None
        else int(str(state.get("d5_max_retries")))
    )
